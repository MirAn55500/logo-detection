...
async def test_if_sent_image_then_label_and_cropped_image_appear_in_response(
    ...
    assert (
        label_expected.lower() in text.lower()
    ), f"Expected label {label_expected} in response"

    # Проверяем, что обрезанное изображение и ближайшее похожее изображение в ответе
    assert (
        '<img src="data:image/png;base64,' in text
...
async def test_if_sent_faulty_image_then_error_appear_in_response(client):
    ...
    with patch.object(yolo_model, "predict", yolo_mock), patch.object(
        feature_model, "forward", feature_mock
    ):
        response = await client.post("/", data=form)
